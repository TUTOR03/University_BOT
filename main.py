...
main_url = 'https://128bf72f8c47.ngrok.io'
		...
		if(last_time['ok'] and last_time['status']):
			if('register_' in user_mes):
				...
				if(acc_type == 1):
					...
					reply_mes = 'Чтобы стать исполнителем отправьте заявку на почту....'
				...
				if(user_mes == '/start'):
					...
					if(user_status['register']):
						if(user_status['acc_type'] == 1):
							...
							reply_mes = 'Добро пожаловать исполнитель'
							if(acc_is_admin):
								keyboard = create_default_keyboard([
									['Список заданий 📃'],
									['Мои задания 📚'],
			...
			if(acc_status == 'waiting'):
				...
				if(user_mes == '/help'):
					...
					if(ans['ok']):
						...
						reply_mes = 'У вас нет доступа к этой команде\nЧтобы переключится на исполнителя отправльте запрос через /help'
						bot.send_message(user_id, reply_mes)
					return({'ok':True})
